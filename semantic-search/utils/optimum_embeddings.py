...
#        model: Optional[Any] = None,
        ...
        if provider == 'CUDAExecutionProvider':
            device = 'cuda'
            kwargs = {"provider_options":None}
        elif provider == 'CPUExecutionProvider':
            device = 'cpu'
            kwargs = {"provider_options":None}
        elif provider == 'TensorrtExecutionProvider':
            device = 'cuda'
            cache_path = os.path.join(os.sep,"tmp",os.path.split(folder_name))
            kwargs = {"provider_options":{"trt_engine_cache_enable": True, "trt_engine_cache_path": cache_path}}
        else:
            raise ValueError('Incorrect Execution Provider specified.')
        self._model = ORTModelForFeatureExtraction.from_pretrained(folder_name, use_io_binding=True, provider=provider, **kwargs)
        self._tokenizer = tokenizer or AutoTokenizer.from_pretrained(folder_name)
        self._device = device or infer_torch_device()
        self.emd_size = self._model.config.hidden_size
